...
static int scan_time = 60; //最长蓝牙扫描时间
...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks {
    ...
    void onResult(BLEAdvertisedDevice advertisedDevice) {
         ...
         )
      {
        ...
        delay(500);
        Serial.println("advertisedDevice stop...");
        advertisedDevice.getScan()->stop();  //此句放在最后，否则 blue_doConnect = true; 可能来不及执行

        Serial.println("init pServerAddress...");
...
bool Manager_blue_to_hc08::connectToServer(BLEAddress pAddress) {
  Serial.println("A.连接蓝牙地址: ");
  Serial.println(pAddress.toString().c_str());


  Serial.println("B.开始连接");
...
bool Manager_blue_to_hc08::blue_connect_sendmsg(String txt, bool quick)
{
  ...
  //pBLEScan->start 查找蓝牙设备是很费时间的，只需要执行一次即可，这样可以节省连接时间！

  if (quick)
  {
    //上次找到过蓝牙地址，跳过蓝牙扫描,节省时间
    ...
    {
      pBLEScan->start(scan_time);  //阻塞式查找蓝牙设备
    }
  }
  else
    pBLEScan->start(scan_time);  //阻塞式查找蓝牙设备
  ...
  while (blue_doConnect == false)
  {
    Serial.print(".");
    delay(1000);

    //20秒为查找上限
    if ((millis() / 1000 - sendmsg_time) > 20)
      break;
  }
  ...
  if (blue_doConnect == true) {
    ...
    blue_connected = connectToServer(*pServerAddress);
